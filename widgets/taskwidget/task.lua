...
local function worker(data,args)
	...
	--end
	if data.depends and type(data.depends) == "string" then
		local deps = {}
		for w in (data.depends .. ","):gmatch("([^,]*),") do 
			--local item = uuids[w]
			table.insert(deps,w)
		end
		data.depends = deps
	end
	if data.due and type(data.due) == "string" then
		task._due_string = data.due
		task.due = todate(data.due)
	end
	if data.wait and type(data.wait) == "string" then
		task._wait_string = data.wait
		task.wait = todate(data.wait)
	end
	return task
end
