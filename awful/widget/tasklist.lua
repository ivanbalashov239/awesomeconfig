...
local function tasklist_label(c, args)
    ...
    local tasklist_only_icon = args.tasklist_only_icon or theme.tasklist_only_icon or false
    ...
    return not tasklist_only_icon and text or '', bg, status_image, not tasklist_disable_icon and c.icon or nil
end
...
-- @param screen The screen we are drawing on.
-- @return true if c is in a selected tag on screen, false otherwise
--function tasklist.filter.matchedtags(c, screen)
    ---- Only print client on the same screen as this widget
    --if c.screen ~= screen then return false end
    ---- Include sticky client too
    --if c.sticky then return true end
