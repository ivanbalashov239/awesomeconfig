...
menu = {
    ...
    settings={
	    title = "hintmenu",
	    interval = 0.1,  -- in percentages of box
	    lines = 2,
	    linewidth = 0.8, -- in prec of screen weight
	    lineheight = 0.8, -- --||--
	    position = "center", -- hardcoded center, in future: bottom, top, left, light
	    exitonpress = true,
    },
}
...
function menu.result(args)
    local functions	= args.functions or {}
    local title		= args.settings.title		or menu.settings.title
    local interval	= args.settings.interval	or menu.settings.interval
    local lines		= args.settings.lines + 1	or menu.settings.lines
    local linewidth	= args.settings.linewidth	or menu.settings.linewidth
    local lineheight	= args.settings.lineheight	or menu.settings.lineheight
    local position	= args.settings.position	or menu.settings.position
    local exitonpress	= args.settings.title		or menu.settings.title

    local screen = args.screen or capi.mouse.screen
    local screengeom = capi.screen[screen].workarea
    local lw = screengeom.width * linewidth
    local lh = screengeom.height * lineheight
    --need position feature
    local linestartx = screengeom.width * (1 - linewidth) / 2
    local linestarty = screengeom.height * (1 - lineheight) / 2
    local linenumber = #functions / lines
    local lastlinenumber = linenumber + #functions%lines
    local lineheight = lh / lines
    local layout = wibox.




    local hintindex = {} -- Table of functions with the hint letter as the keys

    for i,thisitem in pairs(functions) do 
...
setmetatable(menu, { __call = function(_, ...) return menu.result(...) end })
